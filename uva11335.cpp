...
Suppose they have a distance of 5. Then the optimal way is decrease this distance by 1 2 2. If the distance is 7, the optimal way to decrease the distance could be either 1 2 2 2 or 1 1 2 3. If D = 8, the optimal way is 1 2 2 3. How to decide the optimal way? The answer is simple after some consideration. Find the largest N such that sum(1, ... , N) <= D < sum(1, ..., N+1). Then, just add D - sum(1, ... , N) to the sequence 1, ... , N. Note that, D is not the initial distance between the cop and the theif. It's the distance between them when they have the same speed. (This indicates that let the cop go beyond the theif and let the theif catch up the cop is not optimal. Since for the cop to go beyond the theif, the cop needs faster speed. For the theif to catch up the cop, the theif needs larger speed. Decreasing the speed takes at least 1 second. Let alone the distance can shrink by only 1 for each move).
